    ...
    if ~isempty(specialoptions)
        ...
        for i = 1:length(fldnms)
            for j = 1:size(specialoptions.(fldnms{i}),1)
                if ~any(strfind(lower(fldnms{i}),'pep'))
                    specialoptions.(fldnms{i}){j,2} = specialoptions.(fldnms{i}){j,2} + ...
                        msindfix(specialoptions.(fldnms{i}){j,2});
                end
            end
        end
    end
    ...
    for i = 1:length(bonddesname)
        ...
        if ~isempty(tempbonddes)
            bonddescription = [bonddescription;tempbonddes(:,2),...
                repmat(bonddesname(i),size(tempbonddes,1),1),tempbonddes(:,1)];
        end
    end
