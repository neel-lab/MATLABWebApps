function out = sgp1to2a(in,PTMid,varargin)
if(~isempty(varargin))
    if length(varargin) == 1
        if ~isempty(varargin{1})
            massStruct = varargin{1};
            massedits = massStruct.modSym;
            nameedits = massStruct.modName;
        else
            massedits = [];
            nameedits = [];
        end
...
if PTMid == 1
    ...
    for i = length(msstart):-1:1
        thisms = in(msstart(i));
        if ismember(thisms,massedits)
            symidx = strcmpi(massedits,thisms);
            newms_raw = nameedits{symidx};
            cut = regexp(newms_raw,'(');
            if ~isempty(cut)
                newms = newms_raw(1:cut);
            else
                newms = ['(??-?)',newms_raw];
            end
        else
