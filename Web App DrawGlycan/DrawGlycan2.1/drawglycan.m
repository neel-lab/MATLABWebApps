...
defoptname = {'orientation','monosacsize','msperiwidth',...
    ...
    'centerfontweight','originpointoffset'};  % option names
...
defoptvalue = {'left',.5,1,...
    ...
    12,true,[],...
    true,50,11,...
    'normal',[0,0]};  % option default values
            ...
            if ~isempty(options.specialoptions)
                ...
                for i = 1:length(spfieldnames)
                    if strcmpi(spfieldnames{i},'PEPC') || strcmpi(spfieldnames{i},'PEPN')
                        specialoptions.(spfieldnames{i}(4)) = extspecialoptions.(spfieldnames{i});
                    else
                        specialoptions.(spfieldnames{i}) = [specialoptions.(spfieldnames{i});...
                            extspecialoptions.(spfieldnames{i})];
                    end
                end
            end
        case 'P'
...
switch upper(options.workingmode)
    ...
    case 'GP'  % default format: IUPAC
        ...
        for i = 1:length(cleanpepseq)
            pepobj{i} = text(ax,peppos(i),-1,char(double(cleanpepseq(i))),'FontName','Helvetica','VerticalAlignment','top',...
...
if strcmpi(options.workingmode,'gp') || strcmpi(options.workingmode,'p')
    ...
    if isfield(pepoptions,'NGMOD')
        ...
        for i = 1:size(ngmods,1)
            text(ax,peppos(ngmods{i,2}),-2,ngmods{i,1},'FontName','Helvetica','VerticalAlignment','bottom',...
                'HorizontalAlignment','center','fontsize',options.fontsize*1.5);
        end
    end
    ...
    elseif strcmpi(options.workingmode,'p')
        set(ax,'XLim',[peppos(1) - options.aaspacing,peppos(i) + options.aaspacing],'YLim',[-2.5,1]);
    end
end
