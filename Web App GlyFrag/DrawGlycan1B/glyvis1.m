function glyvis1(mspos,bondmap,linkage,bondescription,mspara,directionseq,identity,options)
% GLYVIS: Glycan visualizer is the main function for displaying glycans
%
% Syntax:
% glyvis(mspos,bondmap,linkage,bondbreak,mspara,directionseq,options)
%
% Input:
...
%         linktyp = '\beta';
    ...
    %     typx = mspos(i,1) - msdistance * linkdist * cos(alpha - options.linkinfotheta/180*pi);
    %     typy = mspos(i,2) - msdistance * linkdist * sin(alpha - options.linkinfotheta/180*pi);
    %     numx = lastmonosac(1) + msdistance * linkdist * cos(alpha + options.linkinfotheta/180*pi);
    %     numy = lastmonosac(2) + msdistance * linkdist * sin(alpha + options.linkinfotheta/180*pi);
            ...
            elseif strcmpi(breaktype{j},'U')
                if msaddtextrot  % vertical glycan
                    text(fig,mspos(i,1) - options.monosacsize*1.02/2,mspos(i,2),...
                        ...
                        'verticalalignment','bottom','fontsize',options.fontsize * 1.25,'rotation',msaddtextrot);
            ...
            elseif strcmpi(breaktype{j},'D')
                if msaddtextrot  % vertical glycan
                    text(fig,mspos(i,1) + options.monosacsize*1.02/2,mspos(i,2),char(double(breakinfo{j})),...
                        'horizontalalignment','center',...
                        'verticalalignment','top','fontsize',options.fontsize * 1.25,'rotation',msaddtextrot);
                else  % horizontal glycan
                    text(fig,mspos(i,1),mspos(i,2) - options.monosacsize*1.02/2,char(double(breakinfo{j})),...
                        'horizontalalignment','center',...
