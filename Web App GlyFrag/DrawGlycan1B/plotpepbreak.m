...
%     set(ax,'Units','characters')
    ...
    delete(t);
    letterheight = diff(ylim)/fontheight*0.6;
%     pixelperunit = height/(diff(ylim)+1);
%     letterheight = options.fontsize*5 * 23/pixelperunit/12;
    ...
    for i = 1:length(unibreakpos)
        ...
        pepbreaktcp(1,2) = min(posy(2),pepbreaktcp(1,2));
