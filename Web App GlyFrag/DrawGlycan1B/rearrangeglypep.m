...
% monosac
%% Empirical parameter for pixel - fontsize conversion
letterwidthunit = DrawGlycanPara.letterwidthpxl/DrawGlycanPara.pixelsperunit;
...
%         adduct = plotinfoout{1}.ADDUCT;
%         adducttext = '';
%         for i = 1:size(adduct,1)
%             adducttext = [adducttext,adduct{i,1},' '];
%         end
%         adductright = plotinfoout{1}.adducttextpos(1) + (length(adducttext)-1) * letterwidthunit;
%     else
%         adductright = [];
%     end
...
%             end
%             adductLR = [min(plotinfoout{i}.bracketspos(:,1)),plotinfoout{i}.adducttextpos(1) + (length(adducttext)-1) * letterwidthunit];
%         else
%             adductLR = [];
%         end
        if ismember('cbitem',fldnames)
...
%         if ismember('bracketspos',fldnames)
%             plotinfoout{i}.bracketspos = plotinfoout{i}.bracketspos + repmat([rootpos(i),0],size(plotinfoout{i}.bracketspos,1),1);
%         end
%         if ismember('adducttextpos',fldnames)
%             plotinfoout{i}.adducttextpos = plotinfoout{i}.adducttextpos + repmat([rootpos(i),0],size(plotinfoout{i}.adducttextpos,1),1);
%         end
        if ismember('cbitem',fldnames)
