...
safezoneind = logical(zeros(size(bondmap,1),1));
iscombined = zeros(size(bondmap,1),1);
for i = 1:length(forkseq)  % one fork at a time
    ...
    frklocalsafeind = logical(zeros(size(bondmap,1),1));  % recording of finished fork and anticollision operations
    brnhlocalsafeind = logical(zeros(size(bondmap,1),1));  % recording of finished branch, also used by in-branch PM anticollision
    for j = 1:length(thesebranches)
        ...
        brnhlocalsafeind = logical(zeros(size(bondmap,1),1));
    end
