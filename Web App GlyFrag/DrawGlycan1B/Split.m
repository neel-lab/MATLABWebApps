...
% tempallmonopos = zeros(size(allmonoind));
tempallmonopos = [allmono(allmonoind(1,:));allmono(allmonoind(2,:))];
% tempallmonopos = allmono(allmonoind);
allparenpairpos = [posparenpairstart;posparenpairend];
if isempty(allparenpairpos)
    allparenpairpos = [0;0];
end
% allparenpairpos = [[posparenpairstart,length(temp_IUPAC + 1)];[posparenpairend,length(temp_IUPAC + 2)]];
ind = 1;
allmonopos = [];
