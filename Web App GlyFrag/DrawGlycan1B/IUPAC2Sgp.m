...
if rem(length(BracIndex),2) ~= 0
    x = 0;
end

% Convert to Sgp format
if(version==1)
    Sgpout = Convert2Sgp(MonoStr,MonoIndex,BracStr,BracIndex,massedits);
elseif(version==2)
...
function [MonoStr,MonoIndex,BracStr,BracIndex] = Split2(IUPAC)
% 'Mono(b?-?)' or 'Monob-?' or 'Monob?'
% MonoPattern = DrawGlycanPara.regexp_MonoPattern_Split; % consider using mass to present the monosaccharide
MonoPattern = '[A-Z_a-z_0-9]*\((.*?)\)';
% '[' or ']'

BracketPattern = DrawGlycanPara.regexp_BracketPattern;
[MonoStr,MonoIndex,MonoIndexend] = regexp(IUPAC,MonoPattern,'Match','start','end');
[BracStr,BracIndex] = regexp(IUPAC,BracketPattern,'Match','start');
msind = zeros(1,length(IUPAC));
msind(BracIndex) = 1;
for i = 1:length(MonoIndex)
    msind(MonoIndex(i):MonoIndexend(i)) = 1;
end
specmsind = find(msind == 0);
if any(specmsind)
    tempspecmsind = diff(specmsind) == 1;
    tempspecmsind = [tempspecmsind 0];
    consecrecorder = find(tempspecmsind,1);
    currentind = consecrecorder + 1;
    while currentind <= length(tempspecmsind)
        continu = 1;
        while continu == 1
            if tempspecmsind(currentind) == 1
                consecrecorder = [consecrecorder,currentind];
                currentind = currentind + 1;
            else
                consecout = consecrecorder;
                continu = 0;
                tempspecmsind(consecrecorder) = 0;
                currentind = find(tempspecmsind,1);
                consecrecorder = currentind;
                currentind = currentind + 1;
            end
        end
        MonoStr = [MonoStr,IUPAC(specmsind([consecout,max(consecout)+1]))];
        MonoIndex = [MonoIndex,min(specmsind(consecout))];
    end
end
end

function Sgpout = Convert2Sgp(MonoStr,MonoIndex,BracStr,BracIndex,MassEdits)
...
Pen = {'Ara','Lyx','Xyl','Rib'};
if isempty(MassEdits)
    Othersin3 = {'Kdn','NeuGc','NeuAc','Neu5Ac','Neu','Bac','LDMan',...
        'Kdo','Dha','DDMan','MurNAc','MurNGc','Mur','Api','Fru',...
        'Tag','Sor','Psi'};
    Othersin1 = {'k','g','s','s','u','b','L',...
        'o','y','D','A','M','m','i','c','t','r','i'};
else
    Othersin3 = cell(1,(18+length(MassEdits.modName)));
    Othersin1 = cell(1,(18+length(MassEdits.modName)));
    Othersin3(1:18) = {'Kdn','NeuGc','NeuAc','Neu5Ac','Neu','Bac','LDMan',...
        'Kdo','Dha','DDMan','MurNAc','MurNGc','Mur','Api','Fru',...
        'Tag','Sor','Psi'};
    Othersin1(1:18) = {'k','g','s','s','u','b','L',...
        'o','y','D','A','M','m','i','c','t','r','i'};
    Mresidues = cellfun(@(x) extractBefore(x,'('),MassEdits.modName,'UniformOutput',false);
    Mempty = cell2mat(cellfun(@(x) isempty(x),Mresidues,'UniformOutput',false))==1;
    Mresidues(Mempty) = MassEdits.modName(Mempty);
    Othersin3(19:end) = (Mresidues)';
    Othersin1(19:end) = (MassEdits.modSym)';
end
Sgpout     = [];
...
for i = 1:length(WholeSeq)
    ...
    if(~isempty(find(ithIndex-MonoIndex==0,1)))
        ...
        elseif(sum(cellfun(@(x)matchMono(x,ithCellStr),Othersin3)==1))
            ...
            if length(MonoId) > 1
                selectName = Othersin3(MonoId);
                selectSin = Othersin1(MonoId);
                selectId = cell2mat(cellfun(@(x) strcmpi(x,ithCellStr(1:end-6)),selectName,'UniformOutput',false));
                SgpSymbol = selectSin{selectId};
            else
